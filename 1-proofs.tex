...
\begin{document}
    ...
    \section{Mathematical Truth}
        ...
        \textbf{Discuss:} What is truth? How do we determine the truth? In the context of society, the truth may be determined by:
        \begin{itemize}
            \item 
            Reason or deduction,
        ...
        In mathematics, it is much simpler. Truth is just a value assigned to a statement.
        ...
        \subsection{Statements}
            ...
            For the above statements, $P, Q$ are true while $R, S$ are false.
        ...
        \subsection{Predicates}
            ...
            The truth of a statement can be \textit{predicated} on another variable. For example, consider
            \begin{align}
                P(x) &:= \text{$2x$ is even.}
            ...
            We call $P(x)$ and $Q(x)$ \textbf{predicates}, and the collection of $x$ the \textbf{universe of discourse,} which we describe with set theory.

            \textbox{
            \begin{definition}[Set membership]
                \label{def:inf-function}
                    A \textbf{set} is a collection of objects, which are the set's members.
                    
                    We write $x \in S$ to say that ``$x$ is a member of the set $S$".
            \end{definition}
            }
            
            For example, let $S = \{x, y, z\}$ and assume $a, b, c, d$ are all ``different" objects (none of them are equal to each other). Then $x \in S = \{x, y, z\}$ is a true statement, and $d \in S = \{x, y, z\}$ is false.
    ...
    \subsection{Proof by contrapositive}
        ...
        We can also prove statements with \textbf{indirect proofs}, which do not show the truth of the statement from only direct implications. The first type of indirect proof we will look at is \textbf{contrapositive proof}.
        
        A conditional statement $P \implies Q$ is logically equivalent to the \textbf{contrapositive statement}, $\neg Q \implies \neg P$ (which can be verified with a truth table). Therefore we can prove the original conditional statement by proving the contrapositive instead.
        
        Let's look at the same example as before, but using contrapositive proof.
        ...
        \begin{proof}(Contrapositive)
            ...
            If $x$ is not odd then it is even, then there is an integer $a$ where
            \begin{align}
                x = 2a
                \implies
                x^2 - 6x + 5
        ...
        \begin{proof}(Contrapositive)
            We can informally prove the contrapositive of the proposition, which is that
            \begin{align}
                  f'(x^*)
                  = \frac{f(x^* + dx) - f(x^*)}{dx}
                  = \frac{f(x^*) - f(x^* - dx)}{dx}
            ...
            \begin{align}
                ...
                &\implies
                = \frac{f(x^* + dx)}{dx} > \frac{f(x^*)}{dx}
                \\
                &\implies
                f(x^* + dx) > f(x^*),
            \end{align}
            which means $f(x^*)$ is not the maximum.
            ...
            In both cases $f'(x^*) > 0$ and $f'(x^*) < 0$, we see that $f(x^*)$ is not a maximum. Therefore $f'(x^*) \ne 0$ always implies $f(x^*)$ is not the maximum, which is the contrapositive as required.
        \end{proof}
    ...
    \subsection{Proof by contradiction}
        ...
        This is valid since the statement $P \iff (\neg P \implies Q) \land (\neg P \implies \neg Q)$ is a tautology, which says that if $P$ is true, then assuming $\neg P$ is true causes a contradiction from its implications.
        ...
        \begin{proof}(Contradiction)
            For the sake of contradiction, suppose $x^2$ is even and $x$ is not even. Then $x$ is odd, and there exists some number $a$ where $x = 2a + 1$. Then
            \begin{align}
                x
                = 2a + 1
                \implies
        ...
        This definition implies that if a number $\hat p$ is \textbf{not divisible by any prime}, then $\hat p$ is a prime.
        ...
        \begin{proof}(contradiction
            Suppose for the sake of contradiction that there are finitely many primes. In other words, we can exhaustively list the $n$ many finite primes in ascending order:
            \begin{align}
                p_1,\ p_2,\ p_3,\ p_4, ...,\ p_n = 2,\ 3,\ 5,\ 7, ...,\ p_n,
            \end{align}
            
            where $p_n$ is the largest prime number.
            ...
            The number $p^*$ is not divisible by any of other other primes since $p^* \div p_i$ always has remainder $1$, which is not 0. Since $p^*$ is not divisible by any prime, then $p^*$ must be a prime. But $p^*$ is not included in the list of primes $p_1, p_2, ..., p_n$, since $p^* > p_n$, the largest prime.
            
            Thus there cannot be such a finite list, as you can always construct a number not divisible by any prime like above, which by definition makes it a prime not included in the list of primes---a contradiction.
        \end{proof}
        
        \begin{exercise}
            For any natural number $n$, $n + 1$ is a natural number, and $n + 1 \ne 0$. Prove that there are infinitely many natural numbers.
        \end{exercise}
    ...
    \subsection{Proof by induction}
        ...
        \end{enumerate}
        The inductive step allows us to show that $P(b) \implies P(b+1) \implies ... \implies P(n-1) \implies P(n)$, which is equivalent of stating that $P(b) \implies P(n)$.
        
        Then combining the base case, we have the following tautology:
        \begin{align}
